...
| R482 | API reference: `ekokod tool openapi --format markdown` renders the generated OpenAPI document to `docs/api/reference.md`; `docs/api/openapi.json` is committed; `make api-docs` regenerates both, `make check-api-docs` fails when they drift (same pattern as `check-generate`). | none |
