...
// priors are load_profile readings regardless of the boundary kind, because
// those are the readings dense enough to have one (R55).
//
// Precondition (R92): every element of resets must be KindReset and every
// element of priors must be KindLoadProfile. Derive does not simply trust
// the caller: an element of the wrong kind is skipped rather than used as
// evidence — filterKind drops a wrong-kind reset before it is ever
...
// slice. A caller should still pre-filter, both for clarity and because the
// skip path costs a little more than trusting homogeneous input.
//
// Precondition (the same one SelectBoundary documents): both resets and
// priors must be supplied sorted ascending by TS. Derive does not sort or
// copy them — doing that unconditionally made repeated derivation over a
...
// Values and Suspect left nil.
//
// R92(1): start.TS.Equal(end.TS) alone is enough to emit nothing, even when
// start and end are different kinds — a load_profile start and a reset row
// landing on the same instant measure nothing (no time passed), so a plain
// difference between them would be meaningless even though it computes.
//
// Source is end.Kind, except when end.Kind is KindReset: a reset row is
// evidence of a physical meter event, not a boundary a caller should
...
// NOT w. A reset exactly at end.TS is used as a segmentation point; one
// before w.From but after start.TS is used; one after end.TS is ignored. A
// reset at start.TS is NEVER a segmentation point, even when start is
// itself that reset row: it stays outside the open lower bound of the
// window. This does NOT mean a reset at start.TS is invisible to Derive —
// see R92(2) immediately below, which inspects it for a different purpose
// than segmentation.
//
// R92(2) (amends R90; needs no separate implementation of R90 itself): the
// start-side mirror of R90. When a reset row's TS equals start.TS and start
// is not itself that reset row, start's value at the shared instant is
// exactly as ambiguous as end's value is at a reset landing on end.TS
// (R90) — it could be the replacement meter's first reading, or the OLD
// meter's last cumulative reading landing on the same instant. Derive
...
// the tie-break exists so the rule is well defined regardless.
//
// Fast path: when no reset falls anywhere in the segmentation window AND
// no reset sits exactly at start.TS (so R92(2) has nothing to check
...
// previous reset's TS < prior.TS < this reset's TS). R91: if that one
...
// a missing before-reset value NEVER contributes zero.
//
// R92(4) (amends R91's precedence, which must hold at both boundaries, not
// only the end one): ReasonMeterReset wins over ReasonNegativeDelta for a
// register whenever BOTH problems occur anywhere in the window, regardless
// of which one a naive left-to-right scan would meet first. Concretely: a
// negative segment earlier in time does not short-circuit the scan — Derive
// keeps examining every remaining segment (and, once the loop finishes
// without a reset-evidence problem, the R90 end-of-window check) before
// deciding. If any segment lacks its before-reset value, the register's
// reason is ReasonMeterReset even though an earlier segment was negative
// first; only when no reset-evidence problem exists anywhere does the FIRST
// negative segment's own delta win, as before. Consumption is the sum of
// the segment deltas when nothing is suspect.
//
// R90: when the last in-window reset carrying a register's value sits
// exactly at end.TS and end is not itself a reset row (end.Kind !=
// KindReset — this is checked for ANY non-reset end kind, including
// KindBilling, not only KindLoadProfile), end is ambiguous evidence — it
// could be the post-reset meter's first reading, or the OLD meter's last
// cumulative reading landing on the same instant, since operator- and
// provider-entered resets tend to fall on round local times that are also
// where billing buckets end and load-profile readings are stamped. Derive
...
// and R92(4) extends that same precedence to a negative segment found
// anywhere earlier in the window, not only one that would coincide with
// this check.
//
// R93 (reverses an earlier, unruled draft): a reset row that participates
// in the window — inside (start.TS, end.TS], or at the start.TS/end.TS
// instant per R92(2)/R90 — but has NO value for a register that BOTH
// boundary readings report carries UNUSABLE evidence for that register: the
// register is ReasonMeterReset-suspect, never a plain difference across the
// reset. This holds even when another reset row in the same window does
// carry the register — a single partial row is enough, because we cannot
// tell what happened to that register at the meter event the partial row
// records. A register that NEITHER boundary reports is untouched by this
// rule and stays nil with no suspicion, as always (it is simply
// unreported). Plain difference across a reset only happens when NO reset
// row participates in the window at all for either boundary (the true "no
// reset evidence" case) — see the fast path above and the
// len(windowResets)==0 branch below.
//
...
// "a reset exists but could not be applied" from "no reset at all".
//
...
func Derive(w Window, start, end *Reading, resets []Reading, priors []Reading) Derivation {
	...
	if start.TS.Equal(end.TS) {
		// R92(1): a zero-width pair never emits, whatever the kinds.
		return d
	}
	...
	if end.Kind == KindReset {
		// A reset row is evidence, not a boundary; do not attribute the
		// derived row to it.
		d.Source = start.Kind
	}

	// R92: never trust a reset row of the wrong kind. priors's kind
	...
	// R93: a boundary reading of KindReset is always treated as reset
	// evidence at its own instant, whether or not the caller also included
	// it in resets — a caller must not have to duplicate a reset-kind
	// boundary into resets for Derive to see it as evidence. end is
	// appended (not prepended) because inEvidenceWindow already returns
	// entries in ascending TS order and end.TS is the window's maximum
	// instant — this only fires when no element of resets already sits at
	// end.TS, so the slice stays sorted. On the start side, no analogous
	// append is needed: deriveRegister's R92(2) check is gated on
	// start.Kind != KindReset, so a start.Kind == KindReset boundary is
	// never inspected as ambiguous evidence regardless of startReset's
	// value — startReset is still set here for documentation symmetry with
	// the ruling's wording, but it changes no behaviour.
	...
	if len(windowResets) == 0 && startReset == nil {
		// Fast path: no reset evidence in the window for any register,
...
// (startTS, endTS], in the same relative order as resets. The input
...
// Derive — costing one O(n) scan and no allocation. Only a genuinely mixed
// slice pays for a fresh copy (R92): Derive must not trust an element of
// the wrong kind as evidence. Relative order is preserved, so
// sorted-ascending input stays sorted, and the input slice is never
// mutated.
...
func deriveRegister(reg Register, start, end *Reading, windowResets []Reading, startReset *Reading, priors []Reading) (*decimal.Decimal, *Suspicion) {
	...
	// R92(2): the start-side mirror of R90. A reset row at start.TS is
	// never a segmentation point — see inEvidenceWindow's open lower
	...
	//
	// R93: resetRows counts this row whenever it is inspected here, whether
	// or not it carries reg — a reset row that PARTICIPATES at start.TS but
	// has no value for reg is unusable evidence for reg, not silent
	// non-evidence, since both boundaries report reg (checked above).
	...
	// R93: a reset row inside the segmentation window that has no value
	// for reg is unusable evidence for reg — never a plain difference
	// across it — because both boundaries report reg (checked above) and
	// we cannot tell what happened at that meter event for reg. This fires
	// even when OTHER resets in the same window do carry reg: one partial
	// row is enough to make reg suspect. After this loop, windowResets is
	// safe to use directly as every element (if any) is guaranteed to
	// carry reg.
	...
	if len(windowResets) == 0 {
		// No reset evidence anywhere in the window for ANY register (R93
		// above already handles "resets exist but omit reg"), so
	...
	for _, r := range regResets {
		...
		if before == nil {
			// R91: a missing before-reset value is never a zero
			// contribution, and never falls back to an older reading.
			// R92(4): do not return yet. meter_reset must win over a
		...
		if delta.IsNegative() {
			if negative == nil {
				// The FIRST failing segment's own difference, in time
	...
	if resetProblem {
		// R92(4): some segment's before-reset value was missing, so
	...
	// on purpose: meter_reset must win even when the mismatch would also
	// read as a negative difference, and R92(4) extends the same
...
//
// R91: if that one reading exists but does not report reg, its nil IS the
// answer. priorValueBefore never reaches further back to an earlier
...
//
// R92: a candidate reading of the wrong kind (not KindLoadProfile) is
// skipped as if it were never in priors at all, and the search steps back
// to the next candidate — it is never trusted as before-reset evidence,
// and it never counts as "the single last reading" that R91's no-fallback
// rule pins to.
//
// The lookup starts with a binary search: the first index with
