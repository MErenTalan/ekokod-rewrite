...
// already filtered readings to the kind(s) it wants considered (R64).
// An unsorted slice gives an unspecified result.
//
// The lookup is a binary search (sort.Search for the first reading after
// bound, then step back one) rather than a linear scan — over a billing
// year of 15-minute readings and hourly bounds, the linear version cost
// about 1.75s of CPU for one request; BenchmarkBillingYearHourly records
// the current cost. Ties: when several readings share the same
