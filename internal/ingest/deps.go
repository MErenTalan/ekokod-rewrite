...
// satisfied structurally by *credentials.Service; ingest declares its own
// narrow interface rather than importing internal/credentials so the two
// packages stay independently buildable (internal/credentials depends on
// ingest.Enqueuer, not the reverse).
...
// would. *generation.Accumulator implements this for pm5340, reconciling
// MeterReading.IntervalGenerationKwh against generation_anchors.
...
// range. R17 declared the task ahead of any caller; R73's enqueue call site
// (FetchReadings' finishRun and failFetchRun paths, gated by
// shouldEnqueueConsumptionRefresh) calls it with the run's affected range —
// never the requested window — whenever Deps.ConsumptionRefresh is non-nil,
// Options.ConsumptionRefreshEnabled is true, and R100(1)'s other conditions
// hold. worker/wiring.go's adapter wraps *job.Client with
// job.NewConsumptionRefreshTask. A nil Deps.ConsumptionRefresh disables the
// enqueue seam entirely.
...
type Deps struct {
	...
	// ConsumptionRefresh enqueues consumption.refresh — see
...
type Options struct {
	...
	// site (R73): EKOKOD_CONSUMPTION_REFRESH_ENABLED, default
