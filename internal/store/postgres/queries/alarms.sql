...
where company_id = sqlc.arg(company_id)
  ...
  and (cardinality(sqlc.arg(types)::text[]) = 0 or type::text = any(sqlc.arg(types)::text[]))
  and (sqlc.narg(is_enabled)::boolean is null or is_enabled = sqlc.narg(is_enabled))
  and (not sqlc.arg(filter_analyzer)::boolean
       or exists (select 1 from alarm_analyzers aa where aa.alarm_id = alarms.id and aa.analyzer_id = sqlc.arg(analyzer_id)))
...
select * from alarm_analyzers where alarm_id = sqlc.arg(alarm_id) order by analyzer_id;

-- name: AlarmAnalyzerDeleteForAlarm :exec
delete from alarm_analyzers where alarm_id = sqlc.arg(alarm_id);

-- name: AlarmAnalyzerInsert :exec
insert into alarm_analyzers (alarm_id, analyzer_id) values (sqlc.arg(alarm_id), sqlc.arg(analyzer_id));
...
select * from alarm_channels where alarm_id = sqlc.arg(alarm_id) order by channel, target;

-- name: AlarmChannelDeleteForAlarm :exec
delete from alarm_channels where alarm_id = sqlc.arg(alarm_id);

-- name: AlarmChannelInsert :exec
insert into alarm_channels (alarm_id, channel, target) values (sqlc.arg(alarm_id), sqlc.arg(channel), sqlc.arg(target));
...
values (gen_random_uuid(), sqlc.arg(alarm_id), sqlc.narg(analyzer_id), sqlc.arg(triggered_at),
...
values (sqlc.arg(alarm_id), sqlc.arg(bill_id))
on conflict do nothing
returning true;

-- name: AlarmMarkIsolarForwarded :one
insert into isolar_forwarded_alarms (plant_id, alarm_ref, sent_at)
values (sqlc.arg(plant_id), sqlc.arg(alarm_ref), sqlc.arg(sent_at))
on conflict do nothing
returning true;
