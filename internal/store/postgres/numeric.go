...
// the provider did not report the register, or the tariff does not define that
// price — states that must stay distinguishable from zero all the way up to
// the model struct, which is why every nullable numeric maps to
...
// decimalPtrToNumeric is decimalToNumeric for a NULLABLE column: a nil pointer
// becomes SQL NULL.
//
