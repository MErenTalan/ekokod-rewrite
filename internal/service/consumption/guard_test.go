...
// A behavioural R61 guard that only proves a cross-wired fallback
// dereferenced a nil field never inspects the TYPE of the field itself, so
// a defensive extra field (say, `Readings store.ReadingRepository` on
// AnalyticsDeps) plus a nil-guarded fallback call would leave the suite
// GREEN as long as the fallback's own code path was never exercised. This
// file instead guards with a reflection walk over the struct's field
// TYPES: no runtime call is needed to prove the field should not exist at
// all.

// TestAnalyticsDepsHasNoReadingOrAnomalyRepositoryField is the reflection
...
// TestBillingDepsHasNoAnalyticsRepositoryField is the reflection guard for
...
// TestBillingStructItselfHasNoForbiddenField prove a guard walking only the
// *Deps structs would miss an unexported field of a forbidden type added
// directly on Analytics or Billing (bypassing *Deps entirely). The
// reflection walk is the same; only the root type differs
...
// TestFindForbiddenFieldCatchesAFuncTypedField proves a field typed
...
// must be caught: without a reflect.Func case, such a field stays
// invisible. findForbiddenField (below) is the pure,
// non-t.Fatalf-calling half of the guard, so this test can assert the
// case IS detected without ever failing this test itself.
...
// TestFindForbiddenFieldCatchesAMapKeyTypedField proves a Map case that
// walks only Elem(), never Key(), would miss a field typed
// `map[store.ReadingRepository]bool` (the forbidden repository used as a
// map KEY rather than a value). findForbiddenField walks both Key() and
// Elem() for reflect.Map, so this test — mirroring
// TestFindForbiddenFieldCatchesAFuncTypedField's shape for the Func case —
// must be caught.
...
// TestFindForbiddenFieldCatchesANarrowerConsumerDefinedInterface proves a
// field typed as a NARROWER interface naming only one
...
// TestFindForbiddenFieldCatchesAnInterfaceMethodReturningTheForbiddenType
// proves a field typed as an interface whose method set
...
// TestFindForbiddenFieldCatchesAChanOfTheForbiddenType proves a field typed
// `chan store.ReadingRepository` must be caught: without a reflect.Chan
// case in the walk's switch, such a field stays invisible, exactly like
// reflect.Func would without its own case.
...
//
// Without a reflect.Func case, a field typed
// `func() store.ReadingRepository` (a lazy accessor a defensive fallback
// could call instead of storing the repository directly) would stay
// invisible to this guard. Every parameter and every return type of a Func
// field is walked exactly like any other field's type.
//
// Without walking both Key() and Elem() for reflect.Map, a field
// typed `map[store.ReadingRepository]T` (the forbidden repository used as
// the map's KEY rather than its value) would stay invisible.
//
// `cur == iface || cur.Implements(iface)` alone only ever
// catches a field typed as the FULL forbidden interface (or something wider
...
func findForbiddenField(typ reflect.Type, forbidden ...reflect.Type) (string, bool) {
	...
	walk = func(cur reflect.Type, p string) {
		...
		for _, iface := range forbidden {
			...
			// A narrower interface a forbidden repository could still
		...
		case reflect.Interface:
			// An interface field whose own method
