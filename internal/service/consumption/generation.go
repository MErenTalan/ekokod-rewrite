...
// itself non-nil — the same guard Balance and ExportRows apply. A suspect
// export register's Indexes entry comes back nil too (soundIndex,
// sound.go), exactly as ExportRows blanks a suspect register's closing
// index — the two helpers must not diverge on this rule.
...
// filterSoundIndexes is filterSoundValues' sibling for r.Indexes: each kept
// register goes through soundIndex (sound.go)
